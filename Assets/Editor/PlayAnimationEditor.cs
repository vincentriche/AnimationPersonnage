...
public class PlayAnimationEditor : EditorWindow
{
    ...
    {
        PlayAnimationEditor window = GetWindow<PlayAnimationEditor>();
        window.Show();
    }
    ...
    {
        if (trajectories == null || bodyJoints == null || clipToPlay == null)
            return;
    ...
    public void OnGUI()
    {
        ...
        if (clipToPlay != null)
        {
            ...
            bool all = toggleAll;
            toggleAll = EditorGUILayout.Toggle("Toggle All", toggleAll);
            if (toggleAll != all)
            {
                for (int i = 0; i < toogleTrajectories.Count; i++)
                    toogleTrajectories[bodyJoints[i].name] = toggleAll;
            }
